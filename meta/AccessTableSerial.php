 ...
 *
 * Load and (more importantly) save serial data
 *
 * @package dokuwiki\plugin\struct\meta
 */
class AccessTableSerial extends AccessTable {

    /**
     * AccessTableSerial constructor.
     *
    ...
    public function clearData() {
        if(!$this->rid || !$this->pid) return; // no data

        /** @noinspection SqlResolve */
        $sql = 'DELETE FROM ? WHERE pid = ? AND rid = ?';
        $this->sqlite->query($sql, 'data_'.$this->schema->getTable(), $this->pid, $this->rid);
        $this->sqlite->query($sql, 'multi_'.$this->schema->getTable(), $this->pid, $this->rid);
    }
